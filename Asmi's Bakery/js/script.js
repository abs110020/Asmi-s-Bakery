...
load("./datatry.json", 2, "section3", "#section3B", () =>
      ...
      for (let i = 0; i < data.children.length; i++) {
        ...
        for (let j = 0; j < fills.length; j++) {
          ...
          if (type === "IMAGE") {
            ...
            const docWidth = data.width;
            const docHeight = data.height;
...
function initAnimateAway(selector) {
    ...
    if (window.gsap && window.ScrollTrigger) {
        ...
        document.querySelectorAll(selector).forEach((elem) => {
            // Determine the direction in which the element should animate
            // For simplicity, let's say you're animating them horizontally
            let direction = (elem.getBoundingClientRect().left + elem.offsetWidth / 2) > window.innerWidth / 2 ? "+=200" : "-=200"; // adjust the value '200' to whatever distance you want

            gsap.to(elem, {
                x: direction, // this will animate the x position (horizontal)
                ease: "none",
                scrollTrigger: {
                    trigger: "#section1",
                    start: "top top",  // when the top of the element hits the center
                    end: "bottom center", // when the bottom of the element hits the center
...
//document.addEventListener("DOMContentLoaded", initAnimateAway);
